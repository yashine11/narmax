...
export default function AnimeBrowsePage() {
  ...
  useEffect(() => {
    api.get('/api/search/genres').then((response) => setGenres(response.data.genres || [])).catch(() => {});
  }, []);
  ...
  const requestAnimePage = useCallback(async (params) => {
    try {
      return await api.get('/api/tmdb/anime', { params });
    } catch {
  ...
  useEffect(() => {
    ...
    requestAnimePage(buildParams(1))
      .then((response) => {
  ...
  return (
    <div className="min-h-screen bg-black pt-20 pb-32 sm:pt-24">
      <Helmet>
        <title>Anime — NARMAX</title>
        <meta name="description" content="Explore the best trending and top-rated anime series and movies on NARMAX." />
      </Helmet>
      <div className="mx-auto max-w-[1920px] px-4 sm:px-8">
        <div className="mb-8 sm:mb-10">
          <p className="text-xs font-bold uppercase tracking-[0.24em] text-cyan-200">Anime</p>
          <h1 className="mt-2 text-3xl sm:text-5xl font-black text-white tracking-tight">Anime</h1>
        </div>
        ...
        <div className="mt-8">
          {loading || (fetchFailed && results.length === 0) ? (
            <>
              ...
              {fetchFailed && !loading && (
                <div className="mt-8 flex justify-center">
                  <button
                    ...
                    className="rounded-full border border-cyan-300/25 bg-[#03045e]/50 px-6 py-3 text-sm font-semibold text-cyan-100 transition hover:bg-[#1e40af]/45"
                  >
                    Retry anime
                  </button>
                </div>
            ...
            <div className="rounded-[1.4rem] border border-white/10 bg-white/[0.03] px-6 py-10 text-center text-zinc-400">
              No anime titles match the current filters.
            </div>
          )}
        </div>
      </div>
